...
def get_parts_analysis(
    ...
    try:
        ...
        logger.error(f"Error analyzing parts: {str(e)}")
        return []


@log_execution_time
def get_current_merges(client: Client) -> List[Dict[str, Union[str, int, float]]]:
    ...
    try:
        ...
        logger.error(f"Error retrieving current merges: {str(e)}")
...
def get_merge_stats(
    ...
    try:
        ...
        logger.error(f"Error retrieving merge statistics: {str(e)}")
        # Fallback to local query
        fallback_query = f"""
        SELECT 
            toStartOfHour(event_time) AS ts, 
...
def get_part_log_events(
    ...
    try:
        ...
        logger.error(f"Error retrieving part log events: {str(e)}")
        # Fallback to local query
        fallback_query = f"""
        SELECT 
            toStartOfHour(event_time) AS ts, 
...
def get_partition_stats(
    ...
    try:
        ...
        logger.error(f"Error retrieving partition statistics: {str(e)}")
        return []