...
def get_async_insert_stats(
    ...
    try:
        ...
        logger.error(f"Error retrieving asynchronous insert statistics: {str(e)}")
...
def get_insert_written_bytes_distribution(
    ...
    try:
        ...
        logger.error(f"Error retrieving insert written bytes distribution: {str(e)}")
        # Fallback to local query
        fallback_query = f"""
        SELECT 
            bucket, 
...
@log_execution_time
def get_async_vs_sync_insert_counts(
    client: Client,
    days: int = 7
    ...
    try:
        ...
        logger.error(f"Error retrieving async vs. sync insert counts: {str(e)}")
        # Fallback to local query
        fallback_query = f"""
        SELECT 
            toStartOfHour(event_time) AS ts, 
...
def get_recent_insert_queries(
    ...
    logger.info(f"Retrieving recent insert queries for the past {days} days (limit: {limit}, min_count: {min_count})")
    
    try:
        return execute_query_with_retry(client, query)
    except ClickHouseError as e:
        logger.error(f"Error retrieving recent insert queries: {str(e)}")
        # Fallback to local query
        fallback_query = f"""
        SELECT 
            min(event_time) AS min_event_time, 
