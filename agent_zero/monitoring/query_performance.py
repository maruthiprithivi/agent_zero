...
def get_query_duration_stats(
    ...
    logger.info(f"Retrieving query duration statistics for {kind_desc} queries over the past {days} days")
    
    try:
        return execute_query_with_retry(client, query)
    except ClickHouseError as e:
        logger.error(f"Error retrieving query duration stats: {str(e)}")
...
def get_normalized_query_stats(
    ...
    try:
        ...
        logger.error(f"Error retrieving normalized query stats: {str(e)}")
        # Fallback to non-clustered query
        fallback_query = f"""
        SELECT 
            normalized_query_hash, 
...
def get_query_kind_breakdown(
    ...
    try:
        ...
        logger.error(f"Error retrieving query kind breakdown: {str(e)}")
        # Fallback to non-clustered query
        fallback_query = f"""
        SELECT 
            toStartOfHour(event_time) AS ts, 
