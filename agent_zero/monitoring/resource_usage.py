...
def get_memory_usage(
    ...
    try:
        ...
        logger.error(f"Error retrieving memory usage statistics: {str(e)}")
        # Fallback to local query
        logger.info("Falling back to local metric_log query")
        return execute_query_with_retry(
            client,
            base_query.format(table="system.metric_log"),
            settings=settings
        )
...
def get_cpu_usage(
    ...
    try:
        ...
        logger.error(f"Error retrieving CPU usage statistics: {str(e)}")
...
@log_execution_time
def get_server_sizing(client: Client) -> List[Dict[str, Union[str, int]]]:
    ...
    try:
        ...
        logger.error(f"Error retrieving server sizing information: {str(e)}")
        # Fallback to local query
        fallback_query = """
        SELECT 
            hostname() AS server, 
...
def get_uptime(
    ...
    try:
        ...
        logger.error(f"Error retrieving uptime statistics: {str(e)}")
        # Fallback to local query
        fallback_query = f"""
        SELECT 
            toStartOfHour(event_time) AS ts, 
